...
def calculate_statistics(df):
    ...
    # 轮胎压力统计
    print("\n轮胎压力 (PSI):")
    print(f"  左前: {df['tire_pressure_fl'].mean():.2f} (平均) / {df['tire_pressure_fl'].max():.2f} (最大)")
    print(f"  右前: {df['tire_pressure_fr'].mean():.2f} (平均) / {df['tire_pressure_fr'].max():.2f} (最大)")
    print(f"  左后: {df['tire_pressure_rl'].mean():.2f} (平均) / {df['tire_pressure_rl'].max():.2f} (最大)")
    print(f"  右后: {df['tire_pressure_rr'].mean():.2f} (平均) / {df['tire_pressure_rr'].max():.2f} (最大)")
    ...
    max_acceleration = valid_acc[valid_acc > 0].max() if len(valid_acc[valid_acc > 0]) > 0 else 0
    max_deceleration = abs(valid_acc[valid_acc < 0].min()) if len(valid_acc[valid_acc < 0]) > 0 else 0
...
def plot_telemetry_data(df, stats):
    ...
    ax4.pie(gear_values, labels=gear_labels, colors=gear_colors[:len(gear_labels)],
    ...
    tire_data = [df['tire_pressure_fl'], df['tire_pressure_fr'], 
                df['tire_pressure_rl'], df['tire_pressure_rr']]
