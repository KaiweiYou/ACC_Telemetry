    ...
    def update_plot(self, frame):
        ...
        if success and len(self.timestamps) > 1:
            ...
            if len(self.tire_fl) > 0:
                min_pressure = min(min(self.tire_fl), min(self.tire_fr), 
                                 min(self.tire_rl), min(self.tire_rr))
                max_pressure = max(max(self.tire_fl), max(self.tire_fr), 
                                 max(self.tire_rl), max(self.tire_rr))
        ...
        return [self.speed_line, self.rpm_line, 
                self.throttle_line, self.brake_line, self.clutch_line,
                self.tire_fl_line, self.tire_fr_line, self.tire_rl_line, self.tire_rr_line]
